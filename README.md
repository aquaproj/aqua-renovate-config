...
* :thumbsup: `"github>aquaproj/aqua-renovate-config#1.4.0"`
...
{
  "extends": [
    "github>aquaproj/aqua-renovate-config#1.4.0"
  ]
}
```
...
registries:
- type: standard
  ref: v3.107.0 # renovate: depName=aquaproj/aqua-registry
...
{
  "extends": [
    "github>aquaproj/aqua-renovate-config:file#1.4.0(aqua/.*\\.ya?ml)"
  ]
}
```
...
{
  "extends": [
    "github>aquaproj/aqua-renovate-config:installer-script#1.4.0(scripts/.*\\.sh)"
  ]
}
```
